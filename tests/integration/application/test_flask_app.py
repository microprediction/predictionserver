...
def test_attribute_client_manual(attribute_client_manual):
    ...
    res = attribute_client_manual.put(f'/public_attribute/?attribute_type=repository&write_key={write_key}&value=google.com')
    assert res.status_code == 200

    res = attribute_client_manual.get(f'/public_attribute/?attribute_type=repository&code={write_key}')
    assert res.status_code == 200
...
def test_attribute_client_autogen(attribute_client_autogen):
    ...
    res = attribute_client_autogen.put(f'/OwnerPublicAttribute/?attribute_type=repository&write_key={write_key}&value=google.com')
    assert res.status_code == 200

    res = attribute_client_autogen.get(f'/OwnerPublicAttribute/?attribute_type=repository&code={write_key}')
    assert res.status_code == 200
    assert res.data == b"\"google.com\"\n"
