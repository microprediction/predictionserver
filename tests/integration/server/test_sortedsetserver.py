from predictionserver.servermixins.sortedsetserver import SortedSetServer, SortedSetNameGranularity, \
    SortedSetKeyGranularity, SortedSetType
...
def test_leaderboard_not_allowed():
    ...
    name_kwargs = {'write_key': BABLOH_CATTLE,
                   'name': name,
                   'delay': delay,
    ...
    res = server.add_sortedset_value(sortedset_type=sortedset_type,
    ...
    pass
    # Again, less verbose
    assert not server.add_sortedset_value(sortedset_type=sortedset_type,
                                          name_granularity=name_granularity,
                                          key_granularity=key_granularity,
                                          name_kwargs=name_kwargs,
                                          key_kwargs=key_kwargs,
...
def test_leaderboard_allowed():
    ...
    pass
    # Again, less verbose
    assert server.add_sortedset_value(sortedset_type=sortedset_type,
                                      name_granularity=name_granularity,
                                      key_granularity=key_granularity,
                                      name_kwargs=name_kwargs,
                                      key_kwargs=key_kwargs,
    ...
    values = server.get_sortedset_values(sortedset_type=sortedset_type,
                                         name_granularity=name_granularity,
                                         **name_kwargs)
    ...
    # Scale it
    res = server.multiply_sortedset(sortedset_type=sortedset_type,
                                    name_granularity=name_granularity,
                                    name_kwargs=name_kwargs, write_key=write_key,
                                    verbose=True, weight=0.5)
    assert res['success'] == 1
    assert res['success'] == 1
    scaled_values = server.get_sortedset_values(sortedset_type=sortedset_type,
                                                name_granularity=name_granularity,
                                                **name_kwargs)
    ...
    res = server.multiply_sortedset(sortedset_type=sortedset_type,
                                    name_granularity=name_granularity,
                                    name_kwargs=name_kwargs, write_key='wrong key',
                                    verbose=True, weight=0.5)
    assert res['execution'] == 0
    assert res['success'] == 0

    # Delete it all
    del_res = server.delete_sortedset(sortedset_type=sortedset_type,
                                      name_granularity=name_granularity,
                                      name_kwargs=name_kwargs, write_key=write_key, verbose=True)

    pass
    # Put it back incorrectly with invalid key
    invalid_res = server.add_sortedset_value(sortedset_type=sortedset_type,
                                             name_granularity=name_granularity,
                                             key_granularity=key_granularity,
                                             write_key='invalid key',
                                             name_kwargs=name_kwargs, key_kwargs=key_kwargs,
                                             value=value, verbose=True)
    ...
    # Error deleting
    del_res_error = server.delete_sortedset(sortedset_type=sortedset_type,
                                            name_granularity=name_granularity,
                                            name_kwargs=name_kwargs, write_key='wrong_key', verbose=True)
...
def test_authorize():
    ...
    kwargs = {'name': 'my_stream_3.json',
              'delay': 310,
              'write_key': BABLOH_CATTLE}
    # Don't own it yet
    server._delete_ownership_implementation(name=kwargs['name'], write_key=BABLOH_CATTLE)
    res2 = server._authorize_sortedset_change(sortedset_type=SortedSetType.leaderboard,
                                              name_granularity=SortedSetNameGranularity.name_and_delay,
                                              **kwargs)
    assert res2 == False
    # Now we do
    server._set_ownership_implementation(name=kwargs['name'], write_key=BABLOH_CATTLE)
    assert server._authorize_sortedset_change(sortedset_type=SortedSetType.leaderboard,
                                              name_granularity=SortedSetNameGranularity.name_and_delay,
                                              **kwargs)
    server._delete_ownership_implementation(name=kwargs['name'], write_key=BABLOH_CATTLE)
