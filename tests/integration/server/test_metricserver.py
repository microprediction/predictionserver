...
def test_write_key_private_owner_metric():
    ...
    for metric, value in METRICS.items():
        ...
        # SPECIAL CASES
        owner_value = server.get_private_owner_metric(metric=metric, write_key=BABLOH_CATTLE)
        ...
        # INCREMENT
        server.incr_metric(metric=metric, granularity=MetricGranularity.write_key, amount=1.0, write_key=BABLOH_CATTLE)
...
def test_write_key_public_owner_metric():
    ...
    for metric, value in METRICS.items():
        ...
        # INCREMENT
        server.incr_metric(metric=metric, granularity=MetricGranularity.code, amount=1.0, code=code)
...
def test_stream():
    ...
    for metric, value in METRICS.items():
        ...
        value_back = server.get_metric(metric=metric, granularity=MetricGranularity.name,
                                       name=name)
        assert abs(value_back - value) < 1e-6
        value_implicit = server.get_metric(metric=metric, name=name, granularity=MetricGranularity.name )
        ...
        # INCREMENT
        server.incr_metric(metric=metric, granularity=MetricGranularity.name, amount=1.0,name=name)
        value_updated = server.get_metric(metric=metric, write_key=BABLOH_CATTLE)
        assert abs(value_updated - (value + 1.0)) < 1e-6
