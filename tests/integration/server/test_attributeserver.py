...
def test_delete_stream_attribute_manually():
    ...
    # Should probably write a context manager for this as it can be dangerous
    # if OWNERSHIP and NAMES gets out of whack
    ...
    try:
        ...
        description_back = server.get_attribute(
            attribute_type=AttributeType.description,
            granularity=AttributeGranularity.name,
            name=name,
        ...
        assert server.delete_attribute(
            attribute_type=AttributeType.description,
            granularity=AttributeGranularity.name,
            write_key=BABLOH_CATTLE,
            verbose=False,
    ...
    server.client.hdel(server._OWNERSHIP(), name)
    description_back_again = server.get_attribute(
        attribute_type=AttributeType.description,
        granularity=AttributeGranularity.name,
        name=name,
        delay=server.DELAYS[0]
...
def test_delete_horizon_attribute_manually():
    ...
    server.connect(**MICRO_TEST_CONFIG)
    # Create fake ownership for testing
    # Should probably write a context manager for this as it can be dangerous if
    # OWNERSHIP and NAMES gets out of whack
...
STREAM_ATTRIBUTES = {
    AttributeType.description: 'COOL bananas',
