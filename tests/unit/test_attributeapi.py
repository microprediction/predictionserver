...
def test_init():
    ...
    with pytest.raises(TypeError):
        _ = OwnerPublicAttributeApi(dog=6)
...
def test_public_attribute():
    ...
    res = api.api_owner_public_attribute_put(
    ...
    value = api.api_owner_public_attribute_get(
        attribute_type=attribute_type,
        code=api.shash(BABLOH_CATTLE)
    )
    assert value == LI
    # Test that memos are created
    user_memos = api.get_memos(
    ...
    assert memo['allowed'] == 1
    # Overwrite it
    api.api_owner_public_attribute_put(
        attribute_type=attribute_type,
        value='different@different.com',
        write_key=BABLOH_CATTLE
    )
    assert 'different@different.com' == api.api_owner_public_attribute_get(
        attribute_type=attribute_type, code=api.shash(BABLOH_CATTLE)
    )
...
def test_stream_attribute():
    ...
    res = api.api_stream_attribute_put(
        attribute_type=attribute_type,
        name=name,
        write_key=BABLOH_CATTLE,
        value=value
    ...
    res2 = api.api_stream_attribute_put(
        attribute_type=attribute_type,
        name=name,
        write_key=BABLOH_CATTLE,
        value=value
...
def test_private_attribute():
    ...
    res = api.api_owner_private_attribute_put(
        attribute_type=attribute_type, value=EMAIL, write_key=BABLOH_CATTLE
    )
    assert res['success'] == 1
    assert res['context'] == str(ActivityContext.attribute)

    value = api.api_owner_private_attribute_get(
        attribute_type=attribute_type, write_key=BABLOH_CATTLE
    )
    assert value == EMAIL
    # Test that memos are created
    ...
    assert memo['allowed'] == 1
    # Overwrite it
    api.api_owner_private_attribute_put(
        attribute_type=attribute_type,
        value='different@different.com',
        write_key=BABLOH_CATTLE
    )
