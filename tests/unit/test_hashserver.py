from predictionserver.servermixins.hashserver import (
    HashServer, HashNameGranularity, HashKeyGranularity, HashType
)
...
def test_private_hash():
    ...
    server.connect(**MICRO_TEST_CONFIG)
    kwargs = {
        'name': 'my_private_stream.json',
        'delay': 310,
        'write_key': BABLOH_CATTLE
    }
    value = 31
    res = server.set_hash_value(
    ...
    value_back = server.get_hash_value(
        hash_type=HashType.performance,
        name_granularity=HashNameGranularity.write_key,
        key_granularity=HashKeyGranularity.name_and_delay,
        name_kwargs=kwargs,
    ...
    # Scale it
    res = server.multiply_hash(
        hash_type=HashType.performance,
        name_granularity=HashNameGranularity.write_key,
        name_kwargs=kwargs,
        write_key=kwargs['write_key'],
        verbose=True,
        weight=0.5
    )
    assert res['execution'] == 1
    assert res['success'] == 1
    scaled_values = server.get_hash_values(
        hash_type=HashType.performance,
        name_granularity=HashNameGranularity.write_key,
        **kwargs
    )
    ...
    # Delete one entry
    del_val_res = server.delete_hash_value(
        hash_type=HashType.performance,
        name_granularity=HashNameGranularity.write_key,
        key_granularity=HashKeyGranularity.name_and_delay,
        name_kwargs=kwargs,
    ...
    invalid_res = server.set_hash_value(
        hash_type=HashType.performance,
        name_granularity=HashNameGranularity.write_key,
        key_granularity=HashKeyGranularity.name_and_delay,
        name_kwargs=invalid_name_kwargs,
        key_kwargs=kwargs,
        value=value,
        verbose=True
    )
    ...
    # Error deleting
    del_res_error = server.delete_hash(
        hash_type=HashType.performance,
        name_granularity=HashNameGranularity.write_key,
        name_kwargs=kwargs,
        write_key='wrong_key',
...
def test_public_hash():
    ...
    server.connect(**MICRO_TEST_CONFIG)
    kwargs = {
        'name': 'my_other_other_stream.json',
        'delay': 310,
        'write_key': BABLOH_CATTLE,
...
def test_stream_horizon_hash():
    ...
    for hash_type in HashType:
        ...
        wrong_res = server.set_hash_value(
            hash_type=hash_type,
            name_granularity=name_granularity,
            key_granularity=key_granularity,
            name_kwargs=wrong_name_kwargs,
            key_kwargs=kwargs,
            value=value,
            verbose=True
        )
        ...
        pipe = server._pipe_incr_hash_value_implementation(
            pipe=pipe,
            hash_type=hash_type,
            name_granularity=name_granularity,
            key_granularity=key_granularity,
        ...
        pipe.execute()
        incremented_values = server.get_hash_values(
            hash_type=hash_type,
            name_granularity=name_granularity,
            **kwargs
        )
