...
def test_attribute_location():
    ...
    l = ah.attribute_location(attribute=AttributeType.article,
    ...
    for attribute in AttributeType:
        for granularity in AttributeGranularity:
            ...
            l4 = ah.attribute_location(attribute=str(
                attribute), granularity=str(granularity))
...
def test_attribute_key():
    ...
    assert ah.attribute_key(granularity=AttributeGranularity.name_and_delay,
                            name='john.json', delay=17) == 'john.json|17'
    with pytest.raises(TypeError):
    ...
    assert ah.attribute_key(genus=Genus.bivariate,
                            code=real_code) == real_code + '|bivariate'
    assert ah.attribute_key(genus=Genus.trivariate,
                            code=real_code) == real_code+'|trivariate'
    assert ah.attribute_key(granularity=AttributeGranularity.code,
                            write_key=BABLOH_CATTLE) == real_code
    assert ah.attribute_key(granularity=AttributeGranularity.code,
                            code=real_code) == real_code
    with pytest.raises(TypeError):
        assert ah.attribute_key(granularity=AttributeGranularity.code,
                                genus=Genus.bivariate) == real_code
...
def test_attribute_location_by_inference():
    ...
    assert ah.attribute_location_by_inference(
        attribute=AttributeType.article, name='mystream') == 'bingo::attributes::article::name'
    with pytest.raises(TypeError):
...
def test_attribute_location_and_key():
    ...
    loc, key = ah.attribute_location_and_key(
        attribute=AttributeType.article, granularity=AttributeGranularity.name_and_delay, name='john.json', delay=17)
