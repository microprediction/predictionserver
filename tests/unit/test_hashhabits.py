...
def test_hash_location():
    ...
    name = 'banana.json'
    delay = 310
    l = ah.hash_location(hash_type=HashType.performance, name_granularity=HashNameGranularity.write_key, write_key=BABLOH_CATTLE)
    assert l == 'boom::hash::performance::write_key::'+BABLOH_CATTLE
    l1 = ah.hash_location(hash_type=HashType.priority, name_granularity=HashNameGranularity.name_and_delay,name=name, delay=delay)
    ...
    for hash_type in HashType:
        for name_granularity in HashNameGranularity:
            l1 = ah.hash_location(hash_type=hash_type, name_granularity=name_granularity,**kwargs)
            l2 = ah.hash_location(hash_type=str(hash_type), name_granularity=name_granularity,**kwargs)
            l3 = ah.hash_location(hash_type=hash_type, name_granularity=str(name_granularity),**kwargs)
            l4 = ah.hash_location(hash_type=str(hash_type), name_granularity=str(name_granularity),**kwargs)
