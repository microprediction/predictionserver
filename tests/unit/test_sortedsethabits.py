from predictionserver.serverhabits.sortedsethabits import (
    SortedSetType, SortedSetNameGranularity, SortedSetKeyGranularity, SortedSetHabits
)
...
def test_sortedset_location():
    ...
    for sortedset_type in SortedSetType:
        for name_granularity in SortedSetNameGranularity:
            l1 = ah.sortedset_location(
                sortedset_type=sortedset_type,
                name_granularity=name_granularity,
                **kwargs
            )
            l2 = ah.sortedset_location(
                sortedset_type=str(sortedset_type),
                name_granularity=name_granularity,
                **kwargs
            )
            l3 = ah.sortedset_location(
                sortedset_type=sortedset_type,
                name_granularity=str(name_granularity),
                **kwargs
            )
            l4 = ah.sortedset_location(
                sortedset_type=str(sortedset_type),
                name_granularity=str(name_granularity),
                **kwargs
            )
