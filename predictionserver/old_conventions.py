...
class MicroServerConventions(AttributeHabits, MicroConventions):
    ...
    def __init__(self, history_len=None, lagged_len=None, delays=None, max_ttl=None, error_ttl=None,
        ...
        super().__init__(min_len=min_len, min_balance=min_balance,
                         num_predictions=num_predictions, delays=delays)
        ...
        # Number of seconds that set execution error logs are persisted
        self.CONFIRMS_TTL = int(error_ttl or 60 * 60)
        # Number of seconds that set execution error logs are persisted
        self.ERROR_TTL = int(error_ttl or 24 * 60 * 60)
        # Number of seconds that set execution error logs are persisted
        self.CONFIRMS_TTL = int(error_ttl or 60 * 60)
        # Number of error messages to keep per write key
        self.ERROR_LIMIT = int(error_limit or 1000)
        # Number of error messages to keep per write key
        self.CONFIRMS_LIMIT = int(error_limit or 1000)
        self.TRANSACTIONS_LIMIT = 1000

        # User transparent temporal and other config
        # Number of scenerios in a prediction batch
        self.NUM_PREDICTIONS = int(self.num_predictions)
        ...
        # Reserve of credits fed by rare cases when all models miss wildly
        self._RESERVE = self._obscurity + "reserve"
        ...
        # Redundant set of all names (needed for random sampling when collecting garbage)
        self._NAMES = self._obscurity + "names"
        # Prefixes queues of operations that are indexed by epoch second
        self._PROMISES = self._obscurity + "promises" + self.SEP
        # Prefixes queues of operations that are indexed by minute
        self._CANCELLATIONS = self._obscurity + "cancellations" + self.SEP
        ...
        # Prefix to a listing of contemporaneous predictions by horizon. Must be private as this contains write_keys
        self._PREDICTIONS = self._obscurity + self.PREDICTIONS
        ...
        self._SAMPLES = self._obscurity + self.SAMPLES
        # Prefixes temporary values referenced by the promise queue
        self._PROMISED = "promised" + self.SEP
        self._DONATIONS = "donations" + self.SEP  # Write key donations
        self._DONATION_PASSWORD = MicroConventions.shash(
            self._obscurity)  # Write key donation password
        self._DONORS = "donors"
        ...
        # Seconds beyond the schedule time when promise data expires
        self._DELAY_GRACE = int(delay_grace or 5)
        self._DEFAULT_MODEL_STD = 1.0  # Noise added for self-prediction
        # Sizes of neighbourhoods around truth used in countback ... don't make too big or it hits performance
        ...
        self._LEADERBOARD_TTL = int(24 * (60 * 60) * 30)
