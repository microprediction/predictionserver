...
class StatsConventions:
    ...
    def percentile_abscissa():
        ...
        # Deprecated
        return [-2.3263478740408408, -1.6368267885518997, -1.330561513178897,
                -1.1146510149326596, -0.941074530352976, -0.792046894425591,
                -0.6588376927361878, -0.5364223812298266, -0.4215776353171568,
                -0.3120533220328322, -0.20615905948527324, -0.10253336200497987, 0.0,
                0.10253336200497973, 0.20615905948527324, 0.31205332203283237,
                0.4215776353171568, 0.5364223812298264, 0.6588376927361878,
                0.7920468944255913, 0.941074530352976, 1.1146510149326592,
    ...
    def sign_changes(lagged_values):
        return np.nansum([abs(d) > 1.5 for d in np.diff(
            np.sign(list(lagged_values) + [0., 0.]))])

    @staticmethod
    def is_process(lagged_values):
        " Placeholder for something better :-) "
        return StatsConventions.sign_changes(
            np.diff(lagged_values)) > 2 * StatsConventions.sign_changes(lagged_values)
    ...
    def cdf_values(lagged_values: [float], num, as_discrete=None):
        ...
        if as_discrete:
            return StatsConventions._cdf_discrete_values(
                lagged_values=lagged_values, num=num)
        else:
            return StatsConventions._cdf_continuous_values(
                lagged_values=lagged_values, num=num)

    @staticmethod
    def _cdf_continuous_values(
            lagged_values: [float],
            num: int = None,
            ndigits=2,
            digest=None) -> [float]:
        """
        Returns a list of example values the time series might take next, based on
        recent lags

            num         Maximum number of sample values
            decimals    How to round values, which can be useful in reducing the
                        number of calls
            machine     Supply an existing distribution machine to update it rather
        ...
        xs = list(np.diff([0.] + chronological_values)
                  ) if as_process else chronological_values
        for x in xs:
            digest.update(x=x)
        sample_x = [
        ...
        return unique_x if not as_process else [
            chronological_values[-1] + x for x in unique_x]
