...
from predictionserver.futureconventions.leaderboardconventions import (
    ...
    # LeaderboardMemoryDescription,
    LeaderboardConventions,
)
...
class LeaderboardReader(KeyConventions, BaseReader, LeaderboardConventions):
    ...
    def get_leaderboard(
        ...
        """ Get any kind of leaderboard if you know the full name.
            See LeaderboardConventions for ways to construct the name, or use
            get_leaderboard_strict.
        """
        data = {'count': count, 'with_repos': int(with_repos)}
        return self.request_get_json(
            method='leaderboards',
            arg=leaderboard_name,
            data=data,
            throw=throw,
        )
    ...
    def get_leaderboard_strict(
        ...
        """ Retrieve any leaderboard, different style.
               Supply kwargs to popular granularity  (e.g.  name and delay,
               or code and genus)
        """
    ...
    def get_leaderboard_for_horizon(
            self,
            horizon: str = None,
            name: str = None,
            delay: str = None
    ):
        lb_name = self.leaderboard_name_for_horizon(
            horizon=horizon,
            name=name,
            delay=delay
        )
