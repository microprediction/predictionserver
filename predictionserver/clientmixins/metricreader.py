from predictionserver.futureconventions.metricconventions import MetricConventions, MetricType, \
    MetricGranularity
...
class MetricReader(MetricConventions, BaseReader):
    ...
    def get_metric(self, metric: MetricType, granularity: Union[MetricGranularity, str]=None, **kwargs):
        """
                  Supply all arguments implied by the AttributeGranularity, for example name and delay.
                  Or see special cases below.
        """
        granularity = MetricGranularity[str(granularity)] if granularity else self.metric_granularity_inference(
            **kwargs)
    ...
    def get_metrics(self, metric: MetricType, granularity: Union[MetricGranularity, str]=None, **kwargs):
        """
                  Supply all arguments implied by the AttributeGranularity, for example name and delay.
                  Or see special cases below.
        """
    ...
    def get_horizon_volume(self, name: str, delay:int) -> float:
        return self.get_metric(metric=MetricType.volume, granularity=MetricGranularity.name_and_delay,
                               name=name, delay=delay)

    def get_stream_latency(self, name: str ) -> float:
        latest = self.get_metric(metric=MetricType.latest, granularity=MetricGranularity.name_and_delay, name=name)
    ...
    def get_horizon_budget(self, name:str, delay:int ) -> float:
        return self.get_metric(metric=MetricType.budget, granularity=MetricGranularity.name_and_delay,
                               name=name, delay=delay)

    def get_stream_count(self, name:str ) -> float:
        """ Total number of data points ever published """
        return self.get_metric(metric=MetricType.count, granularity=MetricGranularity.name, name=name)

    def get_owner_rating(self, code:str) -> float:
        """ Strength rating. Supply public identity. """
        return self.get_metric(metric=MetricType.rating, granularity=MetricGranularity.code, code=code)
