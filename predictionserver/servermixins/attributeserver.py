...
#
#       hash name                     key
#   attribute::email::write_key       'ea876e8a67...'
#   attribute::budget::name           'cop.json
#   attribute::linkedin::code         'fle81ble...'
#   attribute::prize::code_and_genus  'fle81ble...|bivariate'


class AttributeServer( MemoServer, OwnershipServer):

    def __init__(self, **kwargs):
        super().__init__(**kwargs)

    def get_attribute(self, attribute_type: Union[AttributeType,str],
                      granularity: Union[AttributeGranularity,str]=None, code: str = None,
                      write_key: str = None,
                      name: str = None, delay: int = None, genus: Genus = None):
        """
               granularity is optional but strongly advised !
        """
        attribute_type = AttributeType[str(attribute_type)]
        ...
        return self._get_attribute_implementation(attribute=attribute_type, granularity=granularity, write_key=write_key,
                                                  code=code, name=name, delay=delay, genus=genus)

    def set_attribute(self, attribute_type: Union[AttributeType,str], value: str, write_key,
        ...
        if granularity is None:
            granularity = self.attribute_granularity_inference(code=code, name=name, delay=delay, genus=genus,
                                                               write_key=write_key)
        attribute_type = AttributeType[str(attribute_type)]
        return self._set_attribute_implementation(attribute=attribute_type, granularity=granularity, value=value,
                                                  write_key=write_key, name=name, code=code, delay=delay, genus=genus,
                                                  verbose=verbose)

    def delete_attribute(self, attribute_type: Union[AttributeType,str], write_key, granularity:AttributeGranularity=None,
        ...
        return self._delete_attribute_implementation(attribute=attribute_type, granularity=granularity, write_key=write_key, name=name, code=code,
    ...
    def get_owner_email(self, write_key: str):
        return self.get_attribute(attribute_type=AttributeType.email, granularity=AttributeGranularity.write_key,
                                  write_key=write_key)

    def get_stream_description(self, name: str):
        return self.get_attribute(attribute_type=AttributeType.description, granularity=AttributeGranularity.name,
                                  name=name)

    def get_horizon_description(self, name: str, delay:int):
        return self.get_attribute(attribute_type=AttributeType.description, granularity=AttributeGranularity.name_and_delay,
                                  name=name, delay=delay)

    def get_owner_description(self, code: str):
        return self.get_attribute(attribute_type=AttributeType.description, granularity=AttributeGranularity.code,
                                  code=code)

    def get_owner_repository(self, code: str):
        return self.get_attribute(attribute_type=AttributeType.repository, granularity=AttributeGranularity.code,
                                  code=code)
    ...
    def _set_attribute_implementation(self, attribute: AttributeType, granularity: AttributeGranularity, value,
        ...
        memo = Memo(activity=Activity.set, context=ActivityContext.attribute, attribute=attribute, write_key=write_key,
                    allowed=int(allowed), name=name, value=value)
        if allowed:
            location, key = self.attribute_location_and_key(attribute=attribute, granularity=granularity,
                                                            write_key=write_key, name=name,
                                                             delay=delay, genus=genus)
            execution = self.client.hset(name=location, key=key, value=value)
            memo['execution'], memo['success'] = execution, 1
            assert self.add_memo_as_owner_confirm(memo)
        else:
