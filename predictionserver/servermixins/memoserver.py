from predictionserver.futureconventions.memoconventions import Memo, MemoCategory, MemoGranularity, Activity, \
    ActivityContext, PublicActor
...
class MemoServer(BaseServer):
    ...
    def get_memos(self, category:MemoCategory, granularity:MemoGranularity, **kwargs):
        json_memos = self.get_memos_json(category=category, granularity=granularity, **kwargs)
        return [json.loads(jm) for jm in json_memos]

    def get_memos_json(self, category: MemoCategory, granularity: MemoGranularity, **kwargs):
    ...
    def get_owner_memos(self, category:MemoCategory, write_key:str):
        return self.get_memos(category=category, granularity=MemoGranularity.write_key, write_key=write_key)
    ...
    def get_stream_transactions(self, name: str, delay:int):
        return self.get_memos(category=MemoCategory.transaction, granularity=MemoGranularity.name_and_delay,
                                  name=name, delay=delay)

    def get_system_memos(self, category: MemoCategory, private_actor: PrivateActor):
        return self.get_memos(category=category, granularity=MemoGranularity.private_actor, private_actor=private_actor)

    def get_public_memos(self, public_actor: PublicActor):
        return self.get_memos(category=MemoCategory.public, granularity=MemoGranularity.public_actor,
    ...
    def add_memo_once(self, memo:Memo, category: MemoCategory, granularity:MemoGranularity):
        memos = self.get_memos(category=category, granularity=granularity, **memo.as_dict())
    ...
    def add_memo(self, memo: Memo, category: MemoCategory, granularity: MemoGranularity):
        """
             Remark: client facing functionality should only allow calling this when granularity is write_key
                     Other uses are reserved for the system
        """
        return self.add_memos(memos=[memo], categories=[category], granularities=[granularity])
    ...
    def add_announcement(self, message: str, url: str):
        ...
        return self.add_memo(memo=memo, category=MemoCategory.alert, granularity=MemoGranularity.public_actor)

    def delete_memos(self, category: MemoCategory, granularity: MemoGranularity, **kwargs):
        """ Delete list/stream of memos
            Only possible when location is private
        """
        assert granularity in [MemoGranularity.write_key,
    ...
    def delete_announcements(self):
        return self.delete_memos(category=MemoCategory.alert, granularity=MemoGranularity.alert,
    ...
    def add_memo_as_owner_confirm(self, memo: Memo, success=1, allowed=1, execution=None):
        return self.execute_one(method=self._pipe_add_memo_as_owner_confirm, allowed=allowed,
                                memo=memo, success=success, execution=execution)

    def add_memo_as_owner_warning(self, memo: Memo, success=0, allowed=None, execution=None):
        return self.execute_one(method=self._pipe_add_memo_as_owner_warning, allowed=allowed,
                                memo=memo, success=success, execution=execution, message=None)

    def add_memo_as_owner_and_horizon_transaction(self, memo: Memo):
        return self.execute_one(method=self._pipe_add_memo_as_owner_and_horizon_transaction, memo=memo)

    def add_memo_as_owner_alert(self, memo: Memo):
        return self.execute_one(method=self._pipe_add_memo_as_owner_alert, memo=memo, write_key=None)
    ...
    def add_system_memo(self, memo: Memo, private_actor: PrivateActor, category: MemoCategory):
        self.execute_one(method=self.__add_system_memo, memo=memo, private_actor=private_actor, category=category)

    def add_memo_as_system_warning(self, memo: Memo, private_actor: PrivateActor):
        self.add_system_memo(memo=memo, private_actor=private_actor, category=MemoCategory.warning)

    def add_memo_as_system_error(self, memo: Memo, private_actor: PrivateActor):
        self.add_system_memo(memo=memo, private_actor=private_actor, category=MemoCategory.error)

    def add_memo_as_system_confirm(self, memo: Memo, private_actor: PrivateActor):
        self.add_system_memo(memo=memo, private_actor=private_actor, category=MemoCategory.confirm)

    # ------------------- #
    #   Implementation    #
    # ------------------- #

    def add_memos(self, memos: [Memo], categories: [MemoCategory], granularities: [MemoGranularity]):
        ...
        for memo, category, granularity in zip(memos, categories, granularities):
            pipe = self.__add_memo(pipe=pipe, memo=memo, category=category, granularity=granularity)
    ...
    def __add_memo(self, pipe, memo: Memo, category: MemoCategory, granularity: MemoGranularity, **kwargs):
        ...
        location_new = self.memo_location(category=category, granularity=granularity, **dmemo)
        ...
        if imp_type == MemoImplementation.redis_list:
            if location_old:
                pipe = self.__log_to_list(pipe=pipe, log_name=location_old, ttl=ttl, limit=limit, **dmemo)
            pipe = self.__log_to_list(pipe=pipe, log_name=location_new, ttl=ttl, limit=limit, **dmemo)
    ...
    def _pipe_add_memo_as_owner_error(self, pipe, memo: Memo, success:int=0, allowed:int=None, execution=None, message=None):
        """
          :return:  pipe
        """
        ...
        return self.__add_memo(pipe=pipe, memo=memo, granularity=MemoGranularity.write_key, category=MemoCategory.error)

    def _pipe_add_memo_as_owner_warning(self, pipe, memo: Memo, success=0, execution=None, allowed=None, message=None):
        """
                 :return:  pipe
        """
        memo['success'] = success
        ...
        if execution is not None:
            memo['execution'] = execution
        return self.__add_memo(pipe=pipe, memo=memo, granularity=MemoGranularity.write_key,
                               category=MemoCategory.warning)

    def _pipe_add_memo_as_owner_confirm(self, pipe, memo: Memo, success=1, execution=None, allowed=1):
        """
            :return:  pipe
        """
        memo['success'] = success
    ...
    def _pipe_add_memo_as_owner_and_horizon_transaction(self, pipe, memo):
        """ Add to transaction log for owner, and also the horizon log
             :return:  pipe
        """
        pipe = self.__add_memo(pipe=pipe, memo=memo, category=MemoCategory.transaction,
    ...
    def _pipe_add_memo_as_owner_alert(self, pipe, memo, write_key=None):
        ...
        pipe = self.__add_memo(pipe=pipe, memo=memo, category=MemoCategory.alert,
                               granularity=MemoGranularity.write_key)
        return pipe

    def __add_system_memo(self, pipe, memo: Memo, private_actor: PrivateActor, category: MemoCategory):
        """ System messages are just memos with special actor identities that are impossible to guess """
        memo['private_actor'] = self.private_actor_key(private_actor=private_actor)
        pipe = self.__add_memo(pipe=pipe, memo=memo, category=category, granularity=MemoGranularity.private_actor)
        return pipe
...
if __name__ == '__main__':
    ...
    memo1 = Memo(activity=Activity.set, context=ActivityContext.lagged, message='testing new confirms',
                write_key=FLATHAT_STOAT, data={'care': 7, 'dog': 13})
    memo1['success'] = 1
    pprint(memo1.as_dict())
    server.add_memo_as_owner_confirm(memo=memo1)
    memos = server.get_memos(category=MemoCategory.confirm, granularity=MemoGranularity.write_key,
