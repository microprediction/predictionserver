...
class ScenarioServer(MemoServer):
    ...
    def submit(self, name, values, delay, write_key, verbose=False):
        return self._permissioned_submit_implementation(name=name, values=values, delay=delay, write_key=write_key, verbose=verbose)
    ...
    def _permissioned_submit_implementation(self, name, values, delay, write_key, verbose=False):
        ...
        if self.bankrupt(write_key=write_key):
            ...
            leaderboard = self._get_leaderboard_implementation(variety=LeaderboardGranularity.name_and_delay, name=name,
                                                               delay=delay, readable=False, count=10000)
    ...
    def _msettle(self, names:[str], values:[float], budgets:[float], with_percentiles:bool, write_keys:[str], with_copulas:bool):
        ...
        for name, budget, write_key, sponsor, value in zip(names, budgets, write_keys, sponsors, values):
            ...
            if any(pools):
                ...
                for delay_ndx, delay, pool, participant_set in zip(range(num_delay), self.DELAYS, pools,
                    ...
                    if len(payments):
                        pipe = self.__update_leaderboards_for_horizon(pipe=pipe, name=name, delay=delay, payments=payments, multiplier=budget)
                        leaderboard_names = self.leaderboard_names_to_update(name=name, delay=delay, sponsor=sponsor)
                        write_code = self.shash(write_key)
                        old_leaderboard_names = [self.old_custom_leaderboard_name(sponsor=write_code, name=name),
                                                 self.old_custom_leaderboard_name(sponsor=write_code)]
                        for (recipient, amount) in payments.items():
                            # Record keeping
                            rescaled_amount = budget * float(amount)
                            self._update_balance(pipe=pipe,write_key=recipient, amount=rescaled_amount)
                            ...
                            pipe = self.__incr_performance(variety = PerformanceGranularity.write_key, write_key=write_key, name=name,
                                                           delay=delay, amount=rescaled_amount)
                            pipe.hincrbyfloat(name=self.performance_name_old(write_key=recipient),
        ...
        if with_percentiles and some_percentiles:
            ...
            if z_names:
                result.update(budgets=z_budgets, names=z_names, values=z_curves, write_keys=z_write_keys)
        return result
...
if __name__=='__main__':
    ...
    values = [ server.norminv(p) for p in server.evenly_spaced_percentiles(server.num_predictions) ]
    server.set_scenarios(name='z1~die.json', values=values, write_key=EMBLOSSOM_MOTH,delay=server.DELAYS[0])