...
class LeaderboardServer(LeaderboardHabits, MemoServer):
    ...
    def get_leaderboard(self, granularity:Union[LeaderboardGranularity, str], count=1200, with_repos=False, **kwargs):
        """ Retrieve any kind of leaderboard """
        leaderboard_granularity = LeaderboardGranularity[granularity] if isinstance(granularity, str) else granularity
        return self._get_leaderboard_implementation(granularity=leaderboard_granularity, count=count, with_repos=with_repos,
                                                    **kwargs)

    def leaderboard_authorization(self, granularity:LeaderboardGranularity, **kwargs)->bool:
        assert kwargs.get('write_key'),' Cannot authorize leaderboard operation without write_key'
    ...
    def delete_leaderboard(self, granularity:LeaderboardGranularity, **kwargs):
        ...
        memo = Memo(activity=Activity.delete, context=ActivityContext.leaderboard, granularity=granularity,
                    allowed=allowed, **kwargs)
        if allowed:
            execution = self._delete_leaderboard_implementation(granularity=granularity, **kwargs)
            self.add_memo_as_owner_confirm(memo=memo, execution=execution)
            if execution==0:
                self.add_memo_as_owner_warning(memo=memo, execution=execution, message='Leaderboard did not exist')
        else:
            self.add_memo_as_owner_error(memo=memo, execution=0, message='Did not provide correct write_key associated with the leaderboard')
        return memo

    def scale_leaderboard(self, granularity:LeaderboardGranularity, **kwargs):
        granularity = LeaderboardGranularity[str(granularity)]
        allowed = self.leaderboard_authorization(granularity, **kwargs)
        memo = Memo(activity=Activity.multiply, context=ActivityContext.leaderboard, granularity=granularity,
                    allowed=allowed, **kwargs)
        if allowed:
            execution = self._scale_leaderboard_implementation(granularity=granularity, **kwargs)
            self.add_memo_as_owner_confirm(memo=memo, execution=execution)
        else:
            self.add_memo_as_owner_error(memo=memo, execution=0,
    ...
    def __incr_leaderboards_for_horizon(self, pipe, name, delay, payments, multiplier=1.0):
        ...
        for (recipient, amount) in payments.items():
            ...
            for lb in leaderboard_names:
                pipe = self.__incr_leaderboard(pipe=pipe, leaderboard_name=lb, code=recipient, amount=rescaled_amount)
        return pipe
    ...
    def _delete_leaderboard_implementation(self, granularity, name, **kwargs):
        return self.client.delete(self.leaderboard_name(leaderboard_granularity=granularity, name=name, **kwargs))

    def _get_leaderboard_implementation(self, granularity, count, readable=True, with_repos=False, **kwargs):
        leaderboard_name = self.leaderboard_name(granularity=granularity, **kwargs)
        return self._get_leaderboard_from_name(leaderboard_name=leaderboard_name,with_repos=with_repos,
                                               count=count, readable=readable)

    def _get_leaderboard_from_name(self, leaderboard_name, with_repos, count, readable):
        leaderboard = list(reversed(self.client.zrange(name=leaderboard_name, start=-count, end=-1, withscores=True)))
        if with_repos:
            return self._get_leaderboard_implementation_with_repos(leaderboard, readable)
        return OrderedDict([(MicroConventions.animal_from_code(code), score) for code, score in leaderboard]) if readable else dict(
            leaderboard)
    ...
    def _get_leaderboard_implementation_with_repos(self, leaderboard, readable):
        ...
        return OrderedDict(
            [(MicroConventions.animal_from_code(code), (score, hash_to_url_dict.get(code, None))) for code, score in leaderboard]
    ...
    def _scale_leaderboard_implementation(self, granularity:LeaderboardGranularity, weight:float, **kwargs):
        leaderboard_name = self.leaderboard_name(granularity=granularity, **kwargs)
    ...
    def admin_shrinkage(self):
        ...
        memo = self.add_announcement_of_shrinkage(leaderboard_name=lb_name,execution=execution, weight=weight)
        return memo
...
class LeaderboardMigrationServer(LeaderboardServer, OwnershipServer, MemoServer):
    ...
    def migrate_horizon_leaderboard(self, name, delay):
        ...
        new_retrieved = server.get_leaderboard(granularity=LeaderboardGranularity.name_and_delay,name=name,delay=delay)
        pass
    ...
    def migrate_stream_leaderboard(self, name):
        ...
        self.client.zunionstore(dest=new_lb_name, keys={old_lb_name: 1})
        new_retrieved = server.get_leaderboard(granularity=LeaderboardGranularity.name,name=name)
        pass
    ...
    def migrate_sponsored_leaderboard(self, name):
        ...
        old_lb = self._get_leaderboard_from_name(leaderboard_name=old_lb_name, with_repos=False, count=5, readable=True)
        pprint(old_lb)
        genus = self.genus_from_name(name=name)
        new_leaderboard_name = self.leaderboard_name(granularity=LeaderboardGranularity.sponsor_and_genus,sponsor=sponsor, genus=genus)
        self.client.zunionstore(dest=new_leaderboard_name, keys={old_lb_name: 1})
        new_retrieved = server.get_leaderboard(granularity=LeaderboardGranularity.sponsor_and_genus, genus=genus, sponsor=sponsor, count=5)
    ...
    def old_custom_leaderboard_name(self, sponsor, name=None, dt=None):
        ...
        return self.SEP.join(
            [self.CUSTOM_LEADERBOARD[:-2], sponsor, self.old_lb_cat(name), lb_month(dt)]) + '.json'
...
if __name__=='__main__':
    ...
    pprint(server.get_leaderboard(granularity=LeaderboardGranularity.name_and_delay, name='die.json', delay=server.DELAYS[0]))
