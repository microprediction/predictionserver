...
from predictionserver.api.attributeapi import OwnerPrivateAttributeApi,\
    OwnerPublicAttributeApi, StreamAttributeApi
from flask_restx import Namespace, Api, reqparse
...
def add_attribute_namespaces(api: Api):
    ...
    objs = [OwnerPrivateAttributeApi(), OwnerPublicAttributeApi(), StreamAttributeApi()]
    granularities = [AttributeGranularity.write_key, AttributeGranularity.code, AttributeGranularity.name ]
    docstrings = [ATTRIBUTE_GRANULARITY_EXPLANATIONS[granularity] for granularity in granularities ]
