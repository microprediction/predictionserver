from predictionserver.futureconventions.memoconventions import MemoCategory, Memo, MemoGranularity, MemoConventions
...
class MemoHabits(MemoConventions, ObscurityHabits):
    ...
    def __init__(self, **kwargs):
        ...
        # lkj
        self.MEMO_IMPLEMENTATIONS = { MemoCategory.warning: MemoImplementation.redis_list,
        ...
        self.TRANSACTIONS_TTL = self.MEMO_TTLS[
            MemoCategory.transaction]  # How long to keep transactions stream for inactive write_keys
    ...
    def is_valid_private_actor_key(self, key):
        return key in [ self.private_actor_key(private_actor=pa) for pa in PrivateActor]
    ...
    def memo_location(self, category:MemoCategory, granularity:MemoGranularity, **kwargs):
        """ Determine name of stream or list, such as memos::warning::write_key
            These are mostly straightforward, but for convenience we allow a typed private actor to be supplied
        """
        ...
        if private_actor is not None:
            ...
            if isinstance(private_actor,PrivateActor):
                ...
                assert self.is_valid_private_actor_key(private_actor), 'system actor key is invalid'
            kwargs['private_actor'] = private_actor
        return self.___memos() + str(category) + SepConventions.sep() + granularity.instance_name(**kwargs)
