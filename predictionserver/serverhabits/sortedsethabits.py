...
class SortedSetHabits(SortedSetConventions, ObscurityHabits):
    ...
    def sortedset_location(
            self,
            sortedset_type: Union[SortedSetType, str],
            name_granularity: Union[SortedSetNameGranularity, str],
            **kwargs
    ):
        """
        ...
        return self.obscurity() + \
            SepConventions.sep().join(
                [self._HASH, str(sortedset_type), str(name_granularity), instance]
        )

    def sortedset_key(
            self, key_granularity: Union[SortedSetKeyGranularity, str], **kwargs
    ):
