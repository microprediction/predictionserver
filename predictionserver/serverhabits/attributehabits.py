from predictionserver.futureconventions.attributeconventions import AttributeConventions, AttributeType, \
    AttributeGranularity, ATTRIBUTE_GRANULARITY_EXPLANATIONS
...
def attribute_docstring(granularity: AttributeGranularity):
    ...
    def _doc(method):
        ...
        method.__doc__ = str(api_method) + ' ' + ATTRIBUTE_GRANULARITY_EXPLANATIONS[granularity]
        return method
...
class AttributeHabits(AttributeConventions, ObscurityHabits):
    ...
    def attribute_location(self, attribute: Union[AttributeType, str], granularity: Union[AttributeGranularity, str]):
        ...
        return self.obscurity() + 'attributes' + SepConventions.sep() + str(attribute) + SepConventions.sep() + str(granularity)

    def attribute_key(self, granularity: Union[AttributeGranularity,str]=None, write_key=None,
                      code=None, name=None, delay=None, genus=None):
        """ Determine attribute key, such as   'f10a7ab1e|310  """
        # It is strongly recommended that one supplies granularity, though it can be implied
        granularity = AttributeGranularity[str(granularity)] if granularity else \
            self.attribute_granularity_inference(write_key=write_key, code=code, name=name, genus=genus, delay=delay)
        if 'code' in str(granularity):
            if code is not None:
                ...
                raise TypeError('Must supply code (or write_key) if it is part of attribute granularity')

        return granularity.instance_name(write_key=write_key, code=code, name=name, genus=genus, delay=delay)

    def attribute_location_and_key(self, attribute: AttributeType, granularity: AttributeGranularity, **kwargs):
    ...
    def attribute_location_by_inference(self, attribute: Union[AttributeType, str], **kwargs):
