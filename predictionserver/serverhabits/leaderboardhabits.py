from predictionserver.futureconventions.leaderboardconventions import (
    LeaderboardConventions, LeaderboardGranularity, LeaderboardMemoryDescription
)
...
class LeaderboardHabits(LeaderboardConventions, ObscurityHabits):
    ...
    def __init__(self, **kwargs):
        ...
        # How long to keep leaderboards for inactive streams
        self._LEADERBOARD_TTL = 14 * 24 * 60 * 60
    ...
    def leaderboard_names_to_update(self, name, code, delay):
        ...
        memory_boards = [
            self.leaderboard_name_strict(
                granularity=LeaderboardGranularity.memory,
                memory=memory) for memory in self.LEADERBOARD_MEMORIES.values()
        ]
        ...
        stream_boards = [
            self.leaderboard_name_strict(
                ...
                delay=delay,
                genus=genus,
            ) for granularity in LeaderboardGranularity
        ]
